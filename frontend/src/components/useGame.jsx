...
export function useGame(edgeOfContainer, roomid, enterMatch) {
  ...
  useEffect(() => {
    ...
    if (edgeOfContainer.current && enterMatch) {
      let boxes = Array.from(document.getElementsByClassName("boxes")).map(
        (el) => {
          return {
  ...
  useEffect(() => {
    socket.on("receive_player_state", (playerState) => {
      console.log(playerState[socket.id]["posx"]);
      currentPosition.current = {
        x: playerState[socket.id]["posx"],
        y: playerState[socket.id]["posy"],
      };
      setAngle(playerState[socket.id]["angle"]);
      setBecameZombie(playerState[socket.id]["becomeZombie"]);
      setcurrentPositionOfOthers(playerState);
